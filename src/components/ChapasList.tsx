...
import { Minus, Plus, Edit, Trash2, Download, FileSpreadsheet, FileText } from "lucide-react";
import { 
  DropdownMenu, 
  DropdownMenuTrigger, 
  DropdownMenuContent, 
...
interface ChapasListProps {
  ...
  onExportarChapas: (chapasData: Chapa[], formato: ExportFormat) => void;
}

const ChapasList = ({ chapas, userRole, onDescontar, onAdicionar, onEditar, onExcluir, onExportarChapas }: ChapasListProps) => {
  return (
      ...
      <CardHeader className="flex flex-row items-center justify-between p-4">
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => onExportarChapas(chapas, 'xlsx')}>
                    <FileSpreadsheet className="h-4 w-4 mr-2" />
                    Exportar para XLSX (Excel)
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onExportarChapas(chapas, 'pdf')}>
                    <FileText className="h-4 w-4 mr-2" />
                    Exportar para PDF
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onExportarChapas(chapas, 'csv')}>
                     <FileSpreadsheet className="h-4 w-4 mr-2" />
                    Exportar para CSV
                </DropdownMenuItem>
            </DropdownMenuContent>
